...
const noVerify = config => (config.github.run_hooks ? '' : '--no-verify');
...
const commit = (config) => {
  ...
  exec(`git commit -q ${noVerify(config)} -m "v${config.new_version}"`, { silent: true });
...
const push = (config) => {
  ...
  exec(`git push -q ${noVerify(config)} ${remoteWithCred}`);
