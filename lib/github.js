...
async function uploadAssets() {
  if (this.github.assets.length > 0) {
    const paths = ls(this.github.assets).map(p => path.resolve(p));
    if (paths.length === 0) {
      errorOut('No assets found, nothing to upload!');
    }
    echo('Uploading assets...');
    await Promise.all(paths.map((asset) => {
      const options = {
      ...
      return requestGithub.call(this, 'POST', `${this.github.uploadUrl}?name=${path.basename(asset)}`, options);
    }));
