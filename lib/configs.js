...
const validateOptions = (config) => {
  ...
  required.forEach(([prop, desc]) => {
    if (!valueAt(config, prop)) {
      errorOut(`Missing required option: --${prop} (${desc})`);
    }
  });

  const coerceToStringArray = (prop) => {
    let value = valueAt(config, prop) || [];
    if (typeof value === 'string') {
      value = [value];
    }
    if (!Array.isArray(value)) {
      errorOut(`Invalid type for ${prop}! Must be string or array.`);
    }
    return value;
  };
  config.github.assets = coerceToStringArray('github.assets');
  config.hooks.pre_commit.command = coerceToStringArray('hooks.pre_commit.command');
  config.hooks.pre_release.command = coerceToStringArray('hooks.pre_release.command');
};
