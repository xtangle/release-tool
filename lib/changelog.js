...
const updateChangelog = (config) => {
  ...
  const hashes = { patch: '###', minor: '##', major: '#' }[config.release_type];
  ...
  const changedText = unreleasedSection
    .replace(new RegExp(`\\n${unreleasedHashes}# ([^\\n]*)`, 'g'), `\n${hashes}# $1`)
    .replace(new RegExp(`\\n${unreleasedHashes} \\[?Unreleased]?[^\\n]*`), `${hashes} ${linkLabel} - ${date}`)
    .trim();
